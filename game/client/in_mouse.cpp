...
void CInput::Init_Mouse (void)
{
	...
	m_iCaptureWorkaroundLastMouseX = 0;
	m_iCaptureWorkaroundLastMouseY = 0;
...
void CInput::ResetMouse( void )
{
	...
	// Under RDP or similar capture-limited environments, cursor warping may
	// move the visible cursor but not update the next input position. Track the
	// last real cursor point and use deltas until it escapes the client area.
	...
	*my = m_flAccumulatedMouseYMovement;
	...
	if ( m_rawinput.GetBool() )
	{
		...
		else
		{
			...
			const float flFallbackX = (float)( current_posx - x );
			const float flFallbackY = (float)( current_posy - y );
			if ( flFallbackX || flFallbackY )
			{
				*mx = flFallbackX;
				*my = flFallbackY;
			}

...
C_TFPlayer *GetOwnerFromProxyEntity( void *pEntity )
{
	...
	if ( pBaseEntity )
	{
		...
		if ( pViewModelAttachment && pViewModelAttachment->GetOuter() )
		{
			return ToTFPlayer( pViewModelAttachment->GetOuter()->GetOwnerEntity() );
		}

		CTFViewModel *pViewModel = dynamic_cast<CTFViewModel*>( pBaseEntity );
		if ( pViewModel )
		{
			return ToTFPlayer( pViewModel->GetOwner() );
		}

		CBaseEntity* pOwner = pBaseEntity->GetOwnerEntity();
		while ( pOwner )
		{
			C_TFPlayer *pPlayerOwner = ToTFPlayer( pOwner );
			if ( pPlayerOwner )
				return pPlayerOwner;

			CBaseEntity *pNextOwner = pOwner->GetOwnerEntity();
			if ( !pNextOwner || pNextOwner == pOwner )
				break;
...
//-----------------------------------------------------------------------------
static const int kTFWeaponSkinCompositeCacheVersion = 4;
	...
	virtual void OnBind( void *pC_BaseEntity )
	{
		...
		if ( tfItemDef )
		{
			...
			if ( pDef )
			{
				...
				if ( !rootKV && tfItemDef->GetRemappedItemDefIndex() != tfItemDef->GetDefinitionIndex() )
				{
					rootKV = pDef->GetItemPaintKitDefinitionKV( tfItemDef->GetDefinitionIndex(), nWear );
				}
			}
		}
		...
		if ( rootKV )
		{
			...
			V_sprintf_safe( finalItemName, "%d_%d_wear_%02d_tfport%d", pItem->GetItemDefIndex(), unPaintKitDefIndex, nWear, kTFWeaponSkinCompositeCacheVersion );
	...
	m_mapOverheadEffects( DefLessFunc( const char * ) )
{
	...
	m_bWasThirdPersonBeforeTaunt = false;
	m_bWasThirdPersonOverrideBeforeTaunt = false;
	m_flTauntCamCurrentDist = 0.f;
	m_flTauntCamTargetDist = 0.f;
	m_flTauntCamCurrentDistUp = 0.f;
	m_flTauntCamTargetDistUp = 0.f;
...
void C_TFPlayer::TurnOnTauntCam( void )
{
	...
	m_bWasThirdPersonBeforeTaunt = ( ::input && ::input->CAM_IsThirdPerson() != 0 );
	m_bWasThirdPersonOverrideBeforeTaunt = g_ThirdPersonManager.IsOverridingThirdPerson();
...
void C_TFPlayer::TurnOffTauntCam( void )
{
	...
	const bool bRestoreThirdPerson = m_bWasThirdPersonBeforeTaunt || cl_thirdperson.GetBool() || g_ThirdPersonManager.GetForcedThirdPerson();

	if ( m_bWasThirdPersonBeforeTaunt && !g_ThirdPersonManager.GetForcedThirdPerson() )
	{
		TFPlayerClassData_t *pData = GetPlayerClass()->GetData();
		const Vector vecThirdPersonOffset = pData ? pData->m_vecThirdPersonOffset : vec3_origin;
		m_flTauntCamCurrentDist = m_flTauntCamTargetDist = vecThirdPersonOffset.x;
		m_flTauntCamCurrentDistUp = m_flTauntCamTargetDistUp = vecThirdPersonOffset.z;
		m_TauntCameraData.m_flDist = m_flTauntCamTargetDist;
		g_ThirdPersonManager.SetDesiredCameraOffset( vecThirdPersonOffset );
		TurnOffTauntCam_Finish();
		return;
	}
	...
	{
		if ( bRestoreThirdPerson )
		{
			TFPlayerClassData_t *pData = GetPlayerClass()->GetData();
			m_flTauntCamTargetDist = pData ? pData->m_vecThirdPersonOffset.x : 0.f;
			m_flTauntCamTargetDistUp = pData ? pData->m_vecThirdPersonOffset.z : 0.f;
		}
		else
		{
			m_flTauntCamTargetDist = 0.f;
	...
	if ( !bRestoreThirdPerson || !m_bWasThirdPersonOverrideBeforeTaunt )
	{
...
void C_TFPlayer::TurnOffTauntCam_Finish( void )
{
	...
	const bool bUseLocalThirdPerson = m_bWasThirdPersonBeforeTaunt || cl_thirdperson.GetBool() || g_ThirdPersonManager.WantToUseGameThirdPerson() || g_ThirdPersonManager.GetForcedThirdPerson();
	if ( bUseLocalThirdPerson == false )
	{
		g_ThirdPersonManager.SetOverridingThirdPerson( false );
	...
	else
	{
		...
		if ( pData )
		{
			g_ThirdPersonManager.SetDesiredCameraOffset( pData->m_vecThirdPersonOffset );
			angles[DIST] = pData->m_vecThirdPersonOffset.x;
		}

		::input->CAM_ToThirdPerson();
		ThirdPersonSwitch( true );

		const bool bUseOverrideThirdPerson = !cl_thirdperson.GetBool() && !g_ThirdPersonManager.GetForcedThirdPerson() &&
			( m_bWasThirdPersonBeforeTaunt || m_bWasThirdPersonOverrideBeforeTaunt );
		g_ThirdPersonManager.SetOverridingThirdPerson( bUseOverrideThirdPerson );
	}
...
//	prediction->SetViewAngles( m_angTauntPredViewAngles );
	...
	if ( pWeapon )
	{
		...
		pWeapon->ForceRecreateParticleSystems();
	}
...
void C_TFPlayer::ClientPlayerRespawn( void )
{
	if ( IsLocalPlayer() )
	{
		...
		// Reset taunt camera state without collapsing an active third-person camera.
		m_bWasTaunting = false;
		m_bTauntInterpolating = false;
		m_bWasThirdPersonBeforeTaunt = false;
		m_bWasThirdPersonOverrideBeforeTaunt = false;
		...
		m_flTauntCamCurrentDistUp = 0.f;
		m_flTauntCamTargetDistUp = 0.f;
		m_TauntCameraData.m_flDist = 0.f;
		g_ThirdPersonManager.SetOverridingThirdPerson( false );
...
void C_TFPlayer::OverrideView( CViewSetup *pSetup )
{
	...
	if ( pData && ( g_ThirdPersonManager.WantToUseGameThirdPerson() ||
		( cl_thirdperson.GetBool() && !g_ThirdPersonManager.IsOverridingThirdPerson() ) ) )
	{

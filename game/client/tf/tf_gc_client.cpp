...
void CTFGCClientSystem::WebapiInventoryThink()
{
	...
	case kWebapiInventoryState_RequestAuthToken:
		...
		Msg( "TF inventory: requesting WebAPI auth ticket\n" );
		state.m_hSteamAuthTicket = SteamUser()->GetAuthTicketForWebApi( "tf2sdk" );
		if ( state.m_hSteamAuthTicket == k_HAuthTicketInvalid )
		{
			Warning( "TF inventory: failed to start WebAPI auth ticket request\n" );
			state.Backoff();
			return;
		}
	...
	case kWebapiInventoryState_RequestInventory:
	{
		...
		Msg( "TF inventory: requesting local inventory from %s\n", strUrl.Get() );
		...
		{
			Warning( "TF inventory: failed to send local inventory HTTP request\n" );
			state.Backoff();
			return;
		}
	...
	case kWebapiInventoryState_BuildServerMessage:
	{
		...
		Msg( "TF inventory: built server equipment message with %d equipped item ids\n", msgItems.item_id_size() );
	...
	case kWebapiInventoryState_RequestServerAuthToken:
	{
		...
		CGCClientSharedObjectCache *pSOCache = GetSOCache( SteamUser()->GetSteamID() );
		if ( pSOCache && pSOCache->BIsLocal() && CommandLine() && CommandLine()->FindParm( "-tf_readonly_inventory" ) )
		{
			Msg( "TF inventory: using local read-only inventory handoff, skipping server equipment auth ticket\n" );
			state.m_bufServerAuthToken.Purge();
		...
		Msg( "TF inventory: requesting server equipment auth ticket\n" );
		if ( state.m_hServerAuthTicket != k_HAuthTicketInvalid )
		{
			SteamUser()->CancelAuthTicket( state.m_hServerAuthTicket );
			state.m_hServerAuthTicket = k_HAuthTicketInvalid;
		...
		if ( state.m_hServerAuthTicket == k_HAuthTicketInvalid )
		{
			Warning( "TF inventory: failed to start server equipment auth ticket request\n" );
			state.Backoff();
			return;
		}
	...
	case kWebapiInventoryState_ServerAuthTokenReceived:
	{
		...
		{
			strHexToken.EnsureCapacity( 1 );
			strHexToken.Base()[0] = '\0';
		}
		...
		Msg( "TF inventory: sent equipment request to server\n" );
...
{
	Msg( "TF inventory: server requested equipment refresh\n" );
...
void CTFGCClientSystem::OnWebapiAuthTicketReceived( GetTicketForWebApiResponse_t *pInfo )
{
	...
	{
		Warning( "TF inventory: WebAPI auth ticket request failed with result %d\n", pInfo->m_eResult );
		return;
	}
	...
	{
		Warning( "TF inventory: WebAPI auth ticket had invalid size %d\n", pInfo->m_cubTicket );
		return;
	}
	...
	// Success
	Msg( "TF inventory: received WebAPI auth ticket (%d bytes)\n", pInfo->m_cubTicket );
...
void CTFGCClientSystem::OnWebapiServerAuthTicketReceived( GetTicketForWebApiResponse_t* pInfo )
{
	...
	if ( pInfo->m_eResult != k_EResultOK )
	{
		Warning( "TF inventory: server equipment auth ticket request failed with result %d\n", pInfo->m_eResult );
		return;
	}
	...
	if ( pInfo->m_cubTicket < 0 || pInfo->m_cubTicket > pInfo->k_nCubTicketMaxLength )
	{
		Warning( "TF inventory: server equipment auth ticket had invalid size %d\n", pInfo->m_cubTicket );
		return;
	}
	...
	// Success
	Msg( "TF inventory: received server equipment auth ticket (%d bytes)\n", pInfo->m_cubTicket );
	state.RequestSucceeded();
	state.m_eState = kWebapiInventoryState_ServerAuthTokenReceived;
}
...
void CTFGCClientSystem::OnWebapiInventoryReceived( HTTPRequestCompleted_t* pInfo, bool bIOFailure )
{
	...
	{
		Warning( "TF inventory: local inventory HTTP request failed (successful=%d, status=%d)\n",
			pInfo->m_bRequestSuccessful ? 1 : 0, pInfo->m_eStatusCode );
		SteamHTTP()->ReleaseHTTPRequest( pInfo->m_hRequest );
		return;
	}
	...
	if ( pValues->FindChild( "msg" ) )
	{
		...
		Msg( "TF inventory: local SO cache loaded for %s (%u items)\n",
			userSteamID.Render(), pItemCache ? pItemCache->GetCount() : 0 );
...
void CTFGCClientSystem::SDK_AddServerInventoryInfo( KeyValues* pKV, CGCClientSharedObjectCache* pSOCache )
{
	...
	if ( CommandLine() && CommandLine()->FindParm( "-tf_readonly_inventory" ) && pSOCache->BIsLocal() )
	{
		CMsgSOCacheSubscribed msgLocalSOCache;
		msgLocalSOCache.set_owner( pSOCache->GetOwner().ConvertToUint64() );
		msgLocalSOCache.set_version( pSOCache->GetVersion() );

		CMsgSOCacheSubscribed_SubscribedType *pItemType = msgLocalSOCache.add_objects();
		pItemType->set_type_id( CEconItem::k_nTypeID );

		int nSerializedItems = 0;
		for ( uint32 i = 0; i < pItemCache->GetCount(); ++i )
		{
			CEconItem *pItem = (CEconItem *)pItemCache->GetObject( i );
			...
			if ( !msgItem.SerializeToString( &strItemData ) )
				continue;

			pItemType->add_object_data( strItemData );
			++nSerializedItems;
		}

		std::string strSOCacheData;
		if ( nSerializedItems > 0 && msgLocalSOCache.SerializeToString( &strSOCacheData ) )
		{
			static int s_nLocalReadOnlySOCacheSerial = 0;

			g_pFullFileSystem->CreateDirHierarchy( "tf_readonly_inventory", "MOD" );

			CFmtStr strFilename( "tf_readonly_inventory/%u_%d.bin",
				pSOCache->GetOwner().GetAccountID(), ++s_nLocalReadOnlySOCacheSerial );

			FileHandle_t hFile = g_pFullFileSystem->Open( strFilename.Get(), "wb", "MOD" );
			if ( hFile )
			{
				int nWritten = g_pFullFileSystem->Write( strSOCacheData.data(), strSOCacheData.size(), hFile );
				g_pFullFileSystem->Close( hFile );

				if ( nWritten == (int)strSOCacheData.size() )
				{
					pKV->SetString( "local_readonly_socache_file", strFilename.Get() );
					pKV->SetInt( "local_readonly_item_count", nSerializedItems );
					Msg( "TF inventory: attached local read-only SO cache file %s with %d equipped items (%u bytes)\n",
						strFilename.Get(), nSerializedItems, (uint32)strSOCacheData.size() );
				}
				else
				{
					Warning( "TF inventory: failed to write local read-only SO cache file %s (%d/%u bytes)\n",
						strFilename.Get(), nWritten, (uint32)strSOCacheData.size() );
				}
			}
		}
	}

...
ConVar tf_port_show_rank_panel_without_gc( "tf_port_show_rank_panel_without_gc", "1", FCVAR_ARCHIVE, "Show the main menu PvP rank panel even when the TF GC is unavailable." );
...
void CHudMainMenuOverride::ApplySchemeSettings( IScheme *scheme )
{
	...
	if ( m_pNotificationsScroller && m_pNotificationsScroller->GetScrollbar() )
	{
		auto *pScrollbar = m_pNotificationsScroller->GetScrollbar();
		pScrollbar->SetAutohideButtons( true );
		pScrollbar->SetPaintBorderEnabled( false );
		pScrollbar->SetPaintBackgroundEnabled( false );
		if ( pScrollbar->GetButton(0) )
		{
			pScrollbar->GetButton(0)->SetPaintBorderEnabled( false );
			pScrollbar->GetButton(0)->SetPaintBackgroundEnabled( false );
		}
		if ( pScrollbar->GetButton(1) )
		{
			pScrollbar->GetButton(1)->SetPaintBorderEnabled( false );
			pScrollbar->GetButton(1)->SetPaintBackgroundEnabled( false );
		}
	}
...
void CHudMainMenuOverride::UpdateRankPanelVisibility()
{
	...
	bool bShowRankPanel = bConnectedToGC || tf_port_show_rank_panel_without_gc.GetBool();

	m_pRankPanel->SetVisible( bShowRankPanel );
	m_pRankModelPanel->SetVisible( bShowRankPanel );
	SetControlVisible( "CycleRankTypeButton", bShowRankPanel );
	SetControlVisible( "NoGCMessage", !bShowRankPanel, true );
	SetControlVisible( "NoGCImage", !bShowRankPanel, true );
	UpdateRankPanelType();

...
static bool BTFLaunchedInsecure()
{
	return CommandLine() && CommandLine()->FindParm( "-insecure" );
}
...
bool CTFPartyClient::BCanQueueForMatch( ETFMatchGroup eGroup, CUtlVector< QueueEligibilityData_t >& vecReasons ) const
{
	...
	if ( BTFLaunchedInsecure() )
	{
		lambdaCopyWReason( L"You cannot queue while -insecure is enabled.", k_eDisabledType_System );
		return false;
	}
...
void CTFPartyClient::RequestQueueForMatch( ETFMatchGroup eMatchGroup )
{
	if ( BTFLaunchedInsecure() )
	{
		PartyWarn( "Not requesting queue for %s because -insecure is enabled.\n", GetMatchGroupName( eMatchGroup ) );
		return;
	}
...
void CTFPartyClient::RequestQueueForStandby()
{
	if ( BTFLaunchedInsecure() )
	{
		PartyWarn( "Not requesting standby queue because -insecure is enabled.\n" );
		return;
	}

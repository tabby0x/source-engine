...
static bool BTFReadOnlyInventoryModeEnabled()
{
	const char *pszEnv = getenv( "TF_READONLY_INVENTORY" );
	if ( pszEnv && pszEnv[0] && V_stricmp( pszEnv, "0" ) != 0 )
		return true;

	return CommandLine() && CommandLine()->FindParm( "-tf_readonly_inventory" );
}

static bool BIsSafeLocalReadOnlySOCacheFile( const char *pszFile )
{
	if ( !pszFile || !pszFile[0] )
		return false;

	const char *pszPrefix = "tf_readonly_inventory/";
	if ( V_strncmp( pszFile, pszPrefix, V_strlen( pszPrefix ) ) != 0 )
		return false;

	if ( V_strstr( pszFile, ".." ) || V_strstr( pszFile, ":" ) || V_strstr( pszFile, "\\" ) || V_strstr( pszFile, "//" ) )
		return false;

	return V_strstr( pszFile, ".bin" ) != NULL;
}
...
CTFGCServerSystem::CTFGCServerSystem()
	...
	, m_mapEquipmentRequests( DefLessFunc( CSteamID ) )
{
...
void CTFGCServerSystem::WebapiEquipmentThinkRequest( CSteamID steamID, WebapiEquipmentState_t* pState )
{
	...
	case kWebapiEquipmentState_RequestInventory:
	{
		...
		const char *pszLocalSOCacheFile = pKV->GetString( "local_readonly_socache_file", "" );
		if ( pszLocalSOCacheFile[0] && BTFReadOnlyInventoryModeEnabled() && !engine->IsDedicatedServer() )
		{
			CUtlBuffer bufSOCache;
			if ( BIsSafeLocalReadOnlySOCacheFile( pszLocalSOCacheFile ) &&
				 g_pFullFileSystem->ReadFile( pszLocalSOCacheFile, "MOD", bufSOCache ) &&
				 bufSOCache.TellPut() > 0 )
			{
				CGCClientSharedObjectCache *pSOCache = GetGCClient()->AddLocalSOCache( steamID, bufSOCache.Base(), bufSOCache.TellPut() );
				if ( pSOCache )
				{
					CGCClientSharedObjectTypeCache *pItemCache = pSOCache->FindTypeCache( CEconItem::k_nTypeID );
					Msg( "TF inventory: server loaded local read-only SO cache file %s for %s (%u equipped items, %d advertised, %d bytes)\n",
						pszLocalSOCacheFile, steamID.Render(), pItemCache ? pItemCache->GetCount() : 0,
						pKV->GetInt( "local_readonly_item_count", 0 ), bufSOCache.TellPut() );
				...
				Warning( "TF inventory: server failed to parse local read-only SO cache file %s for %s\n",
					pszLocalSOCacheFile, steamID.Render() );
			}
			else
			{
				Warning( "TF inventory: server failed to read local read-only SO cache file %s for %s\n",
					pszLocalSOCacheFile, steamID.Render() );
			}
		}
		...
		Msg( "TF inventory: server requesting equipment for %s from %s\n", steamID.Render(), strUrl.Get() );
		...
		{
			Warning( "TF inventory: server failed to send equipment HTTP request for %s\n", steamID.Render() );
			state.Backoff();
			return;
		}
...
{
	Msg( "TF inventory: server received client equipment request for %s\n", steamID.Render() );
...
void CTFGCServerSystem::OnWebapiEquipmentReceived( CSteamID steamID, HTTPRequestCompleted_t* pInfo, bool bIOFailure )
{
	...
	{
		Warning( "TF inventory: server equipment HTTP request failed for %s (successful=%d, status=%d)\n",
			steamID.Render(), pInfo->m_bRequestSuccessful ? 1 : 0, pInfo->m_eStatusCode );
		SteamHTTP()->ReleaseHTTPRequest( state.m_hEquipmentRequest );
		return;
	}
	...
	if ( pValues->FindChild( "msg" ) )
	{
		...
		CGCClientSharedObjectTypeCache *pItemCache = pSOCache->FindTypeCache( CEconItem::k_nTypeID );
		Msg( "TF inventory: server SO cache loaded for %s (%u items)\n",
			steamID.Render(), pItemCache ? pItemCache->GetCount() : 0 );
...
void CTFGCServerSystem::SDK_ApplyLocalLoadout(CGCClientSharedObjectCache* pCache, KeyValues* pKVRequest)
{
	...
	Msg( "TF inventory: applied local loadout for %s (%d equipped, %d missing)\n",
		playerSteamID.Render(), nApplied, nMissing );
}

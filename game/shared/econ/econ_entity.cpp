...
static ConVar tf_econ_particles_debug( "tf_econ_particles_debug", "0", FCVAR_DONTRECORD,
	"Print econ attached-particle owner/effect decisions." );
#endif
...
CEconEntity::CEconEntity()
{
	...
	m_bParticleSystemsCreatedInViewModel = false;
...
//-----------------------------------------------------------------------------
// Purpose: Force owner/name re-selection after viewmodel/world transitions.
//-----------------------------------------------------------------------------
void CEconEntity::ForceRecreateParticleSystems( void )
{
	if ( !HasCustomParticleSystems() )
		return;
	...
	if ( pWeapon )
	{
		if ( pWeapon->m_hExtraWearable.Get() )
		{
			pWeapon->m_hExtraWearable->m_nParticleSystemsCreated = PARTICLE_SYSTEM_STATE_NOT_VISIBLE;
			pWeapon->m_hExtraWearable->m_bParticleSystemsCreatedInViewModel = false;
		}

		if ( pWeapon->m_hExtraWearableViewModel.Get() )
		{
			pWeapon->m_hExtraWearableViewModel->m_nParticleSystemsCreated = PARTICLE_SYSTEM_STATE_NOT_VISIBLE;
			pWeapon->m_hExtraWearableViewModel->m_bParticleSystemsCreatedInViewModel = false;
		}
	}
#endif
...
{
	bool bShouldUseViewModelParticles = false;

#if defined(TF_CLIENT_DLL) || defined(TF_DLL)
	CTFWeaponBase *pWeapon = dynamic_cast< CTFWeaponBase* >( this );
	C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();
	C_BasePlayer *pOwner = ToBasePlayer( GetOwnerEntity() );
	if ( pWeapon && pLocalPlayer && pOwner == pLocalPlayer && !pOwner->ShouldDrawThisPlayer() )
	{
		C_BaseViewModel *pLocalPlayerVM = pLocalPlayer->GetViewModel();
		bShouldUseViewModelParticles = pLocalPlayerVM && pLocalPlayerVM->GetOwningWeapon() == this;
	}
#endif

	if ( nState == m_nParticleSystemsCreated )
	{
		bool bDirty = nState != PARTICLE_SYSTEM_STATE_NOT_VISIBLE && bShouldUseViewModelParticles != m_bParticleSystemsCreatedInViewModel;
		...
		if ( pWeapon )
		{
			if ( pWeapon->m_hExtraWearable.Get() )
			{
				bDirty = bDirty || !( pWeapon->m_hExtraWearable->m_nParticleSystemsCreated == nState );
				pWeapon->m_hExtraWearable->m_nParticleSystemsCreated = nState;
			}

			if ( pWeapon->m_hExtraWearableViewModel.Get() )
			{
				bDirty = bDirty || !( pWeapon->m_hExtraWearableViewModel->m_nParticleSystemsCreated == nState );
	...
	m_bParticleSystemsCreatedInViewModel = nState != PARTICLE_SYSTEM_STATE_NOT_VISIBLE && bShouldUseViewModelParticles;
}
...
void CEconEntity::UpdateSingleParticleSystem( bool bVisible, const attachedparticlesystem_t *pSystem )
{
	...
	if ( pszAttachmentName && pszAttachmentName[0] && pEffectOwner && pEffectOwner->GetBaseAnimating() )
	{
	...
	{
		if ( tf_econ_particles_debug.GetBool() )
		{
			Msg( "TF econ particles: missing effect=%s base=%s vm=%d draw_vm=%d owner=%s attachment=%s index=%d visible=%d\n",
				pszSystemName ? pszSystemName : "(null)",
				pSystem->pszSystemName ? pSystem->pszSystemName : "(null)",
				bIsVM ? 1 : 0,
				pSystem->bDrawInViewModel ? 1 : 0,
				pEffectOwner ? pEffectOwner->GetClassname() : "(null)",
				pszAttachmentName ? pszAttachmentName : "(null)",
				iAttachment,
				bVisible ? 1 : 0 );
		}
		return;
	...
	if ( tf_econ_particles_debug.GetBool() )
	{
		Msg( "TF econ particles: visible=%d effect=%s base=%s vm=%d unique_vm=%d owner=%s ent=%d attachment=%s index=%d wm_owner=%s vm_owner=%s draw_vm=%d\n",
			bVisible ? 1 : 0,
			pszSystemName ? pszSystemName : "(null)",
			pSystem->pszSystemName ? pSystem->pszSystemName : "(null)",
			bIsVM ? 1 : 0,
			bHasUniqueVMEffect ? 1 : 0,
			pEffectOwner ? pEffectOwner->GetClassname() : "(null)",
			pEffectOwner ? pEffectOwner->entindex() : -1,
			pszAttachmentName ? pszAttachmentName : "(null)",
			iAttachment,
			pEffectOwnerWM ? pEffectOwnerWM->GetClassname() : "(null)",
			pEffectOwnerVM ? pEffectOwnerVM->GetClassname() : "(null)",
			pSystem->bDrawInViewModel ? 1 : 0 );
	}

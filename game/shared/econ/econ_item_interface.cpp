...
const char *GetPaintKitMaterialOverride( const IEconItemInterface *pItem )
{
	...
	if ( GetPaintKitDefIndex( pItem, &unPaintKitDefIndex ) )
	{
		...
		if ( pPaintKitDef )
		{
			...
			if ( pItemDef && pItemDef->GetRemappedItemDefIndex() != pItem->GetItemDefIndex() )
			{
				pszMaterialOverride = pPaintKitDef->GetMaterialOverride( pItemDef->GetRemappedItemDefIndex() );
				if ( pszMaterialOverride )
				{
					return pszMaterialOverride;
				}

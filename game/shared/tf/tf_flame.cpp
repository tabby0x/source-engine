...
static ConVar tf_flamethrower_particle_debug( "tf_flamethrower_particle_debug", "0", FCVAR_DONTRECORD,
	"Print flamethrower particle owner/control-point diagnostics." );
...
void CTFFlameManager::UpdateWeaponParticleControlPoint( const flame_point_t *pNewestFlame )
{
	...
	Vector vMuzzlePos = vec3_origin;
	QAngle qMuzzleAng = vec3_angle;

	CTFFlameThrower *pFlameThrower = dynamic_cast< CTFFlameThrower* >( m_hWeapon.Get() );
	CBaseAnimating *pRawWeaponOwner = m_hWeapon.Get();
	CBaseAnimating *pVisualOwner = NULL;
	CBaseAnimating *pViewModelAttachment = NULL;
	CBaseAnimating *pMuzzleOwner = pRawWeaponOwner;
	bool bUsingViewModel = false;
	if ( pFlameThrower )
	{
		bUsingViewModel = pFlameThrower->UsingViewModel();
		pVisualOwner = pFlameThrower->GetAppropriateWorldOrViewModel();
		pViewModelAttachment = pFlameThrower->GetViewmodelAttachment();
		if ( pVisualOwner )
		{
			pMuzzleOwner = pVisualOwner;
		}
	}

	if ( m_hMuzzleOwner.Get() != pMuzzleOwner )
	{
		m_hMuzzleOwner = pMuzzleOwner;
		m_nMuzzleAttachment = INVALID_PARTICLE_ATTACHMENT;
	}

	if ( pMuzzleOwner && m_nMuzzleAttachment == INVALID_PARTICLE_ATTACHMENT )
	{
		m_nMuzzleAttachment = pMuzzleOwner->LookupAttachment( "muzzle" );
	}

	bool bHaveMuzzleAttachment = false;
	if ( pMuzzleOwner && m_nMuzzleAttachment != INVALID_PARTICLE_ATTACHMENT )
	{
		bHaveMuzzleAttachment = pMuzzleOwner->GetAttachment( m_nMuzzleAttachment, vMuzzlePos, qMuzzleAng );
	}

	if ( !bHaveMuzzleAttachment )
	{
		CBasePlayer *pOwner = ToBasePlayer( m_hWeapon->GetOwnerEntity() );
		if ( pFlameThrower )
		{
			vMuzzlePos = pFlameThrower->GetVisualMuzzlePos();
			qMuzzleAng = pOwner ? pOwner->EyeAngles() : m_hWeapon->GetAbsAngles();
		}
		else
		{
			vMuzzlePos = m_hWeapon->WorldSpaceCenter();
			qMuzzleAng = m_hWeapon->GetAbsAngles();
		}
	}
	...
	static float s_flNextFlameParticleDebug = 0.0f;
	if ( tf_flamethrower_particle_debug.GetBool() && gpGlobals && gpGlobals->curtime >= s_flNextFlameParticleDebug )
	{
		const Vector vecNewestFlamePos = pNewestFlame ? pNewestFlame->m_vecPosition : vec3_origin;
		const model_t *pRawWeaponModel = pRawWeaponOwner ? pRawWeaponOwner->GetModel() : NULL;
		const model_t *pVisualModel = pVisualOwner ? pVisualOwner->GetModel() : NULL;
		const model_t *pViewModelAttachmentModel = pViewModelAttachment ? pViewModelAttachment->GetModel() : NULL;
		const model_t *pMuzzleOwnerModel = pMuzzleOwner ? pMuzzleOwner->GetModel() : NULL;
		Msg( "TF flame particle: effect=%s weapon=%s raw=%s[%d:%s] visual=%s[%d:%s] vm_attach=%s[%d:%s] cp0_owner=%s[%d:%s] using_vm=%d attach=%d have_attach=%d cp0=(%.1f %.1f %.1f) newest=(%.1f %.1f %.1f) old_particles=%d firing=%d stopped=%d\n",
			m_hParticleEffect ? m_hParticleEffect->GetEffectName() : "(null)",
			m_hWeapon.Get() ? m_hWeapon->GetClassname() : "(null)",
			pRawWeaponOwner ? pRawWeaponOwner->GetClassname() : "(null)",
			pRawWeaponOwner ? pRawWeaponOwner->entindex() : -1,
			pRawWeaponModel ? modelinfo->GetModelName( pRawWeaponModel ) : "(null)",
			pVisualOwner ? pVisualOwner->GetClassname() : "(null)",
			pVisualOwner ? pVisualOwner->entindex() : -1,
			pVisualModel ? modelinfo->GetModelName( pVisualModel ) : "(null)",
			pViewModelAttachment ? pViewModelAttachment->GetClassname() : "(null)",
			pViewModelAttachment ? pViewModelAttachment->entindex() : -1,
			pViewModelAttachmentModel ? modelinfo->GetModelName( pViewModelAttachmentModel ) : "(null)",
			pMuzzleOwner ? pMuzzleOwner->GetClassname() : "(null)",
			pMuzzleOwner ? pMuzzleOwner->entindex() : -1,
			pMuzzleOwnerModel ? modelinfo->GetModelName( pMuzzleOwnerModel ) : "(null)",
			bUsingViewModel ? 1 : 0,
			m_nMuzzleAttachment,
			bHaveMuzzleAttachment ? 1 : 0,
			vMuzzlePos.x, vMuzzlePos.y, vMuzzlePos.z,
			vecNewestFlamePos.x, vecNewestFlamePos.y, vecNewestFlamePos.z,
			m_hOldParticleEffects.Count(),
			m_bIsFiring ? 1 : 0,
			m_hParticleEffect && m_hParticleEffect->m_bEmissionStopped ? 1 : 0 );
		s_flNextFlameParticleDebug = gpGlobals->curtime + 0.25f;
	}
}
...
void CTFFlameManager::OnFiringStateChange()
{
	...
	if ( tf_flamethrower_particle_debug.GetBool() )
	{
		Msg( "TF flame particle: firing_state=%d active_effect=%s weapon=%s old_particles=%d\n",
			m_bIsFiring ? 1 : 0,
			m_hParticleEffect ? m_hParticleEffect->GetEffectName() : "(null)",
			m_hWeapon.Get() ? m_hWeapon->GetClassname() : "(null)",
			m_hOldParticleEffects.Count() );

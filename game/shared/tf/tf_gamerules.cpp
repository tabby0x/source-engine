...
int CTFRadiusDamageInfo::ApplyToEntity( CBaseEntity *pEntity )
{
	...
	else
	{
		...
		nDamageTaken = (int)adjustedInfo.GetDamage();
	}
...
void CTFGameRules::ClientCommandKeyValues( edict_t *pEntity, KeyValues *pKeyValues )
{
	...
	if ( pszCommand && pszCommand[0] )
	{
		...
		else if ( FStrEq( pszCommand, "sdk_inventory" ) )
		{
			...
			if ( pKeyValues->GetString( "local_readonly_socache_file", "" )[0] )
			{
				INetChannelInfo *pNetInfo = engine->GetPlayerNetInfo( pTFPlayer->entindex() );
				if ( engine->IsDedicatedServer() || !pNetInfo || !pNetInfo->IsLoopback() )
				{
					Warning( "TF inventory: rejected local read-only inventory cache from non-loopback client %s\n", steamID.Render() );
					return;
				}
			}
...
void CTFGameRules::RequestClientInventory( CSteamID steamID )
{
	...
	Msg( "TF inventory: requesting client equipment message for %s\n", steamID.Render() );
	CSingleUserRecipientFilter filter( pPlayer );

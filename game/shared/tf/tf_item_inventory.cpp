...
void CTFPlayerInventory::LoadLocalLoadout()
{
	...
	// Loading the saved local loadout is part of inventory initialization. Real
	// user-driven loadout edits still notify through EquipLocal/EquipLocalPreset.
}
...
void CTFPlayerInventory::SOCacheSubscribed( const CSteamID & steamIDOwner, GCSDK::ESOCacheEvent eEvent )
{
	...
	Msg( "TF inventory: TF player cache subscribed for %s (%d items)\n",
		steamIDOwner.Render(), GetItemCount() );

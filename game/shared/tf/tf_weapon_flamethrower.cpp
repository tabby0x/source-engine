...
void CTFFlameThrower::StartFlame()
{
	...
	{
		C_BaseAnimating *pEffectOwner = GetAppropriateWorldOrViewModel();
		const bool bUsingViewModelEffect = UsingViewModel();
		if ( bUsingViewModelEffect )
		{
			C_TFPlayer *pPlayerOwner = GetTFPlayerOwner();
			C_BaseViewModel *pViewModel = pPlayerOwner ? pPlayerOwner->GetViewModel() : NULL;
			if ( pViewModel )
			{
				pEffectOwner = pViewModel;
			}
		}

		if ( pEffectOwner )
		{
			CNewParticleEffect *pEffect = pEffectOwner->ParticleProp()->Create( "pyro_blast", PATTACH_POINT_FOLLOW, "muzzle" );

			if ( pEffect && bUsingViewModelEffect )
			{
...
void CTFFlameThrower::RestartParticleEffect( void )
{
	...
	// New-style flamethrower visuals are custom-origin particles driven by
	// CTFFlameManager. Spawning the same effect as a muzzle-follow particle
	// leaves the flame point control points unset and can create stray
	// first-person emitters near the camera.
	m_FlameEffects.StopEffects();
}

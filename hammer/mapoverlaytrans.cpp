...
void CMapOverlayTransition::CalcBounds( BOOL bFullUpdate )
{
	...
	Shoreline_t *pShoreline = GetShoreManager()->GetShoreline( ( int )( intp )GetParent() );
	if ( pShoreline )
	{
...
{
	GetShoreManager()->RemoveShoreline( ( int )( intp )GetParent() );
}
...
bool CMapOverlayTransition::OnApply( void )
{
	if ( m_bIsWater )
	{
		...
		if ( GetShoreManager()->Init() )
		{
			if ( BuildFaceCaches() )
			{
				m_nShorelineId = ( int )( intp )GetParent();
